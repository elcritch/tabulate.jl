...
function pipe_line_with_colons(colwidths, colaligns)
...
function latex_line_begin_tabular(colwidths, colaligns)
...
function type(string, has_invisible=True)
    ...
    elseif _isnumber(string)
        return float
    elseif isinstance(string, _binary_type)
        return _binary_type
    else 
        return _text_type
	end
...
function format(val, valtype, floatfmt, missingval="")
    ...
    if valtype in [int, _text_type]
        return "{0}".format(val)
    elseif valtype is _binary_type
        try:
            return _text_type(val, "ascii")
        except TypeError:
            return _text_type(val)
    elseif valtype is float
...
function format_table(fmt, headers, rows, colwidths, colaligns)
    ...
    lines = []
    hidden = fmt.with_header_hide if (headers && fmt.with_header_hide) else []
    pad = fmt.padding
